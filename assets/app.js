...
var header = {
  init: function(){
    ...
    $('#menu').slicknav({
      ...
      closedSymbol: '<span class="icon-right-arrow"></span>',
      openedSymbol: '<span class="icon-down-arrow"></span>'
    });
          ...
          $.ajax({
              ...
              success: function (data) {
                $notify_form.fadeOut("slow", function(){
                  $nf.prev('.message').html("Mange tak! Vi giver dig besked, når produktet igen er på vores lager");
                });
              }
          });
        } else {
          $nf.prev('.message').html("Angiv en gyldig e-mailadresse");
        }
        e.preventDefault();
      });
    });
...
$(function() {
  ...
  if ($('.tos_agree').length) {
    $('body').on('click', '#checkout', function() {
      if ($('.tos_agree').is(':checked')) {
        ...
        alert("Venligst accepter handelsbetingelserne og privatlivspolitikken for at fortsætte");
        return false;
      }
    });
  }
  ...
  $contact_form.each(function() {
    ...
    $cf.on('submit', function (e) {
      ...
      if($('input[name="challenge"]').val() !== "true") {
        $.ajax({
          type: $cf.attr('method'),
          url: $cf.attr('action'),
          data: $cf.serialize(),
          success: function (data) {
            ...
            if(data.includes(500)) {
              $('input[name="challenge"]').val('true')
              $cf.submit();
            } else {
              $cf.fadeOut("slow", function(){
                $(this).prev('.message').html("Tak for din tilmelding");
              });
            }
          }
        });
    ...
    $('body').on('submit', '.product_form', function(e) {
      ...
      $.ajax({
        ...
        success: function(item) {
          $addToCartBtn.find('span').text("Lagt i kurven").removeClass('zoomOut').addClass('fadeIn');
          ...
          window.setTimeout(function(){
            var variantSKU = item.sku;
            variantSKU = variantSKU.slice(0, -2);
            var totalPriceAddedToCart = Shopify.formatMoney(item.price, $addToCartForm.data('money-format'));
            totalPriceAddedToCart = totalPriceAddedToCart.replace(/\D/g,'');
            window.dataLayer = window.dataLayer || [];
            window.dataLayer.push({
              'event': 'addedToCart',
              'totalPrice': totalPriceAddedToCart,
              'itemSKU': variantSKU
            });
            $.fancybox.close();
            //var relatedItemsBox = $('[itemtype="http://schema.org/ItemList"]');
            $.fancybox($('<div class="container section cart_notification cart_notification_box" style="width: inherit"><div class="three columns product_image"><img src="' + item.image.replace(/(\.[^.]*)$/, "_grande$1").replace('https:', '') + '" alt="' + htmlEncode(item.title) + '" /></div><div class="eight columns cart_notification_content"><h4><span class="icon-check"></span> Tilføjet til din kurv</h4> <div><strong>' + item.title + '</strong></div><p>' + item.quantity + ' x ' + '<span class="money">' + Shopify.formatMoney(item.price, $addToCartForm.data('money-format')) + '</span></p><p><a class="action_button" href="/cart">Til kassen</a> &nbsp;<span class="continue_shopping">eller <a href="javascript:$.fancybox.close();">Fortsæt med at shoppe </a></span></div></div>'), {padding: padding});
            //$('.cart_notification_box').append('<div class="related_products_notification"><strong>Relaterede produkter</strong></div>');
            $('.cart_notification_box').after('<strong class="relatedHeader">Du vil måske også kunne lide...</strong><div class=""><div class="sixteen columns related_products_notification"></div></div>');
			setTimeout(function(){
              $('.products-related-productlist').clone().prependTo('.related_products_notification');
            },150);
        ...
        error: function(XMLHttpRequest) {
          ...
          $addToCartBtn.removeAttr('disabled').removeClass('disabled');
          $addToCartBtn.find('span').text("Læg i kurven").removeClass('zoomOut').addClass('zoomIn');
        }
      });
...
selectCallback = function(variant, selector) {
  ...
  if(!$productForm.hasClass("init")) {
    ...
    if (variant && variant.available == true) {
      ...
      var calculatedPercentage = ((variant.price * 100)/variant.compare_at_price);
      var calculatedPercentageAfter = 100 - calculatedPercentage;
      if(variant.price < variant.compare_at_price){
        var calculatedPercentageHTML = '<span class="discountAmount" style="color:#f74f4f;font-weight:600;margin-left:6px;">SPAR '+calculatedPercentageAfter.toFixed(0)+' %</span>';
        $('.was_price', $product).html('<span class="money moneytest" style="font-size:20px;font-style:normal;">før ' + Shopify.formatMoney(variant.compare_at_price, $productForm.data('money-format')) + '</span>');
      } else {
        $('.was_price', $product).html("");
        var calculatedPercentageHTML = '';
      }
      ...
      $('.current_price', $product).html('<span class="money" style="font-weight:bold;font-size:16px;">' + Shopify.formatMoney(variant.price, $productForm.data('money-format')) + calculatedPercentageHTML +'</span>');
      $('.add_to_cart', $product).removeClass('disabled').removeAttr('disabled').find('span').text($('.add_to_cart', $product).data('label'));
      $notify_form.hide();
    } else {
      var message = variant ? "Udsolgt" : "Ikke tilgængeligt";
      $('.was_price', $product).html('');
